...
def EditCustomerView(request, id):
    ...
    customer = get_object_or_404(models.Customer, id=request.user.id)
    ...
    if request.method == "POST":
        ...
        form = CustomerForm(
            request.POST or None, request.FILES, instance=customer
        )
        if form.is_valid():
...
def EditCustomerConfigView(request, id):
    customerConfig = get_object_or_404(models.CustomerConfig, customer_id=id)
    ...
    context = {
        ...
        "numberTp": "numberTpForm",
    }
    return render(request, "edit_customer_config.html", context)


def userProfile(request, pk):
    cusotmer = get_object_or_404(models.Customer, id_id=pk)
...
class provider_activity(ListView):
    ...
    def get_queryset(self):
        try:
            with connections["default"].cursor() as cursor:
                cursor.execute(
                    f"select  a.id, avatar, a.name, profit_all, b.loss_all, a.created, b.rank "
...
def planView(request):
    try:
        with connections["default"].cursor() as cursor:
            cursor.execute(
                f"select status from customer where id_id={request.user.id}"
            )
            ...
            cursor.execute(
                f"select plan_name, price, pricewithdisc from plan_price where status=1"
            )
...
class ValidateExtendPlan(View):
    def get(self, request):
        ...
        providerPrice = models.ProviderPrice.objects.get(
            provider=(models.Provider.objects.get(id=provider)),
            planDuration=planDure,
            status=1,
        )
        planPrice = models.PlanPrice.objects.get(
            plan_id=plan, planDuration_id=planDure
        )

        print("*" * 50)
        print(service)
        total_price = providerPrice.price + planPrice.price
        total_price_with_disc = (
            providerPrice.priceWithDisc + planPrice.priceWithDisc
        )
...
def extendPlan(request, id):
    ...
    if request.GET:
        if plan.is_valid():
            ...
            planPrice = models.PlanPrice.objects.get(
                plan_id=id,
                planDuration_id=plan.cleaned_data["planDuration"].id,
            )
...
def dashbord(request):
    account = models.CustomerConfig.objects.filter(
        customer_id=request.user.id
    ).order_by("-id")
    ...
    try:
        with connections["default"].cursor() as cursor:
            cursor.execute(
                f"select  symb,status,updated from signal_order where customer_id={request.user.id}"
            )
            result = cursor.fetchall()
            customer_order = [list(i) for i in result]
            cursor.execute(
                f"select symb,status,updated from signal_order where status = 1"
            )
