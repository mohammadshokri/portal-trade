...
class TicketUpdateView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        t_detail = TicketDetail.objects.filter(id=self.kwargs.get('pk')).last()
...
class TicketDetailExtendView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(TicketDetailExtendView, self).get_context_data(**kwargs)
        print(self.kwargs['pk'])
        # prev_ticket = TicketDetail.objects.filter(ticket=TicketDetail.objects.get(id=self.kwargs['pk']).ticket)
        prev_ticket = TicketDetail.objects.filter(ticket=self.kwargs['pk']).values()
        print(prev_ticket)
