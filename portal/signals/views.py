...
class SignalDetailView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        s_detail = SignalDt.objects.filter(signal=self.kwargs['pk']).values().order_by('tp')
...
class SignalCreateView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            context['signal_detail_formset'] = SignalDetailInlineFormset(self.request.POST)
        else:
    ...
    def form_invalid(self, form, signal_detail_formset):
        return self.render_to_response(
            self.get_context_data(form=form,
                                  signal_detail_formset=signal_detail_formset
                                  )
        )
